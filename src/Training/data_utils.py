...
if __name__ == '__main__':
	tokenizer = RobertaTokenizer.from_pretrained('roberta-large', do_lower_case=False, cache_dir='./')
	candidates = ["still.", "independant.", "good person."]
	max_length = 512
	inputs,labels = handle_underscores("Dana uses Dana's ___ to obtain. Dana is seen as", tokenizer, keywords= ["uses", "obtain"], prefix=False)
	choices = [handle_underscores(cand, tokenizer) for cand in candidates]
	print(choices)
	input_ids = [inputs+cand[0] for cand in choices]
	input_ids = [tokenizer.convert_tokens_to_ids(cand) for cand in input_ids]
	label_ids = [labels+cand[1] for cand in choices]
	...
	[(['Ġstill', '.'], ['Ġstill', -100]), (['Ġindepend', 'ant', '.'], ['Ġindepend', 'ant', -100]), (['Ġgood', 'Ġperson', '.'], ['Ġgood', 'Ġperson', -100])]
	input_ids = [[0, 11014, 2939, 11014, 18, 50264, 7, 6925, 4, 11014, 16, 450, 25, 202, 4, 2], [0, 11014, 2939, 11014, 18, 50264, 7, 6925, 4, 11014, 16, 450, 25, 44913, 927, 4, 2], [0, 11014, 2939, 11014, 18, 50264, 7, 6925, 4, 11014, 16, 450, 25, 205, 621, 4, 2]]
	label_ids = [[-100, -100, 2939, -100, -100, -100, -100, 6925, -100, -100, -100, -100, -100, 202, -100, -100], [-100, -100, 2939, -100, -100, -100, -100, 6925, -100, -100, -100, -100, -100, 44913, 927, -100, -100], [-100, -100, 2939, -100, -100, -100, -100, 6925, -100, -100, -100, -100, -100, 205, 621, -100, -100]]
	
	
	根据output, 可以看出data中包括问题+cand的input token, label和?
'''

#processor做了什么?:
#读取训练集和验证集，处理成一下格式
#{'id':sample['id'], 'context':context, 'correct':label, 'candidates':candidates, 'keywords':keywords}
#再经由convert_examples_to_features传给dataset
#dataset中的data
#为什么要处理成这样呢？
