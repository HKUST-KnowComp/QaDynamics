...
from transformers import (WEIGHTS_NAME, RobertaConfig, RobertaForMaskedLM, RobertaTokenizer, DebertaV2Config, DebertaV2ForMaskedLM, DebertaV2Tokenizer)
...
MODEL_CLASSES = {
	...
	'deberta': (DebertaV2Config, DebertaV2ForMaskedLM, DebertaV2Tokenizer)
}
...
class MyDataset(torch.utils.data.Dataset):
	...
	def __len__(self):
		return len(self.data) #len,list or dataframe?
	...
	for _ in train_iterator:
		...
		for step, batch in enumerate(epoch_iterator):
			...
			if (step + 1) % args.gradient_accumulation_steps == 0:
				...
				if args.local_rank == -1 and args.training_dynamics and args.td_every>0 and (global_step+400)%args.td_every == 0:
					logger.info("Start recording training dyanmics")
					results = evaluate(args, model, tokenizer, train_dataset, (global_step+400)/args.td_every)
					logger.info("The performance on training set is shown above.")
...
def evaluate(args, model, tokenizer, eval_dataset, epoch=0):
	results = {}
	...
	elif args.training_dynamics:
		save_logits(preds.tolist(), args.output_dir+'/td/'+args.logits_file+'_'+str(epoch))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
	...
	processor = myprocessors[task](args) #processor参考data utils文件
	cached_features_file = os.path.join(args.output_dir, 'cached_{}_{}_{}_{}'.format(
		'dev',
		str(args.model_type),
		str(args.max_seq_length),
		str(task)))
	#cache feature file是干嘛的？
	if evaluate and os.path.exists(cached_features_file):
		features = torch.load(cached_features_file)
	else:
		examples = processor.get_dev_examples() if evaluate else processor.get_train_examples() #读取training files
		features = convert_examples_to_features(examples, tokenizer, max_length=args.max_seq_length) #返回data，其中包含token，label和‘mask’
		if evaluate:
...
def main():
	...
	parser.add_argument('--training_dynamics', action='store_true', help="Whether to store the training dynamics")
	parser.add_argument('--td_every', type=int, default=500, help='Obtain the training dyanmics every td_every steps')
